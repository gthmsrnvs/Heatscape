...
const riskLst = {
  temperature: {
    safe: { display: "Low Health Risk", value: 24 },
    caution: { display: "Mild Health Risk", value: 29 },
    danger: { display: "High Health Risk", value: 40 },
  },
  uv: {
    safe: { display: "Low Health Risk", value: 3 },
    caution: { display: "Mild Health Risk", value: 6 },
    danger: { display: "High Health Risk", value: 9 },
  },
  wind: {
    safe: {
      display: "Cooler Wind",
  ...
  humidity: {
    ...
    caution: {
      caption: "Stay hydrated and avoid outdoor activities",
      value: 80,
    },
    danger: {
...
function generateWeatherCards(weather) {
  // Temperature Card
  const temperatureCardContent = document.querySelector(
    ".weatherCardContainer[cardType='temperature'] .weatherCardBottom"
  );
  ...
  // UV index Card
  const uvCardContent = document.querySelector(
    ".weatherCardContainer[cardType='uv'] .weatherCardBottom"
  );
  ...
  // Wind Chill Card
  const windCardContent = document.querySelector(
    ".weatherCardContainer[cardType='wind'] .weatherCardBottom"
  );
  ...
  // Humidity Card
  const humidityCardContent = document.querySelector(
    ".weatherCardContainer[cardType='humidity'] .weatherCardBottom"
  );
...
function calHealthRisk(weather) {
  ...
  Object.keys(riskLst).forEach(
    (type) => (totalHealthRisk += getWeatherRisk(weather, type, false).level)
  );
